(function() {
    ...
    addBtn.addEventListener('click', function() {
        ...
        if(value){ // value 값이 있을경우
            addItemTodo(value); // value값을 채워라
    ...
    function addItemTodo(text) { // value는 text로 들어간다.
        var list = document.getElementById('todo');
        var item = document.createElement('li');
        item.textContent = text; // 추가된 li에 text value가 들어간다.
        ...
        list.insertBefore(item, list.childNodes[0]); // 최신 순으로 위에 정렬
    ...
    function completeItem() {
        var item = this.parentNode.parentNode;
        var parent = item.parentNode;
        var id = parent.id; // todo or complete
